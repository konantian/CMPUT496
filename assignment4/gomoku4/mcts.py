...
def uct_val(node, child, exploration, max_flag): 
    if child._n_visits == 0:
        return float("inf")
    if max_flag:
        black = float(child._black_wins)/child._n_visits + exploration*np.sqrt(np.log(node._n_visits)/child._n_visits)
        # print("black: " + str(black))
        return black
    else:
        white = float(child._n_visits - child._black_wins)/child._n_visits + exploration*np.sqrt(np.log(node._n_visits)/child._n_visits)
...
class MCTS(object):
    ...
    def get_move(self, board, toplay, num_simulation, exploration):
        """
    ...
    def good_print(self, board, node, color, num_nodes):
        ...
        while not node.is_leaf():
            ...
            if move==PASS:
                move = None
            assert cboard.is_legal_gomoku(move, color)
            pointString = self.point_to_string(cboard.size, move)
            cboard.play_move_gomoku(move, color)
